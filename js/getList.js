            ...
            getProductions(page,
                (e, d) => {
                    ...
                    for (let index = 0; index < items.length; index++) {
                        items[index].querySelector("img").src = d[index].imageUrl;
                    }
                    for (let index = 0; index < titleElement.length; index++) {
                        titleElement[index].querySelector("span").innerHTML = d[index].name;
                    }
                    for (let index = 0; index < price.length; index++) {
                        price[index].querySelectorAll("span")[0].innerHTML = "￥" + d[index].nowPrice;
                        price[index].querySelectorAll("span")[1].innerHTML = "￥" + d[index].oldPrice;
                    }
                    for (let index = 0; index < store.length; index++) {
                        store[index].querySelector("span").innerHTML = d[index].storeName;
                    }
                    for (let index = 0; index < sales.length; index++) {
                        sales[index].querySelector("div").innerHTML = "月销" + d[index].sales;
                    }
